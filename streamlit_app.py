...
TRIAL_FILE = "trial_start.txt" # New file for trial timestamp
    ...
    if os.path.exists(TRIAL_FILE):
        with open(TRIAL_FILE, "r") as f:
            try:
                ...
                # Handle case where file might be corrupted or empty
                return None
    return None

def set_trial_start_time():
    with open(TRIAL_FILE, "w") as f:
        f.write(str(time.time()))

def clear_trial_start_time():
    if os.path.exists(TRIAL_FILE):
...
def activate_app(code):
    ...
    if code in codes:
        ...
        with open("activated.txt", "w") as f:
            f.write("activated")
        clear_trial_start_time() # Clear trial info on successful activation
        return True
    return False
...
def main():
    ...
    if not st.session_state.activated:
        ...
        if trial_start_time is None:
            # First time user or trial file deleted
            if st.button("🕒 بدء التجربة المجانية", key="start_trial_button"):
                set_trial_start_time()
                st.session_state.trial_start = time.time() # Update session state for immediate use
                st.success("🎉 بدأت النسخة التجريبية. لديك 5 دقائق. يرجى تحديث الصفحة أو إعادة تشغيل التطبيق.")
                st.experimental_rerun() # Rerun to show active trial immediately
        else:
            # Trial has started previously
        ...
        # Activation input is always present
        ...
        if st.button("🔐 تفعيل التطبيق", key="activate_button"):
            if code and activate_app(code.strip()):
                ...
                st.experimental_rerun() # Rerun to apply activation immediately
            else:
